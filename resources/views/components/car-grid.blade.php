        ...
        <div class="bg-white rounded-lg border border-black/10 hover:shadow-md transition-all">
            ...
            <div class="p-4">
                ...
                <div class="flex items-center gap-1 mb-3">
                    @for ($i = 0; $i < 5; $i++)
                        <svg class="w-4 h-4 {{ $i < 4 ? 'text-yellow-400' : 'text-gray-300' }}" viewBox="0 0 24 24"
                            fill="currentColor">
                            <path
                ...
                <div class="mb-4">
                    ...
                    <div class="text-sm text-gray-500">Rp{{ number_format($car->harga_sewa * 7, 0, ',', '.') }}/minggu
                    </div>
                </div>
                @csrf
                <a href="{{ route('sewa-mobil.create') }}"><button
                        class="w-full py-2 rounded text-white text-sm font-medium bg-gradient-to-r from-[#038EFF] to-[#65BAFF] hover:opacity-90 transition-opacity">Sewa</button></a>
            </div>
        </div>
    @endif
@endforeach
